    ...
    <div className="h-screen w-screen flex items-center justify-center bg-main-bg relative overflow-hidden px-4 select-none font-sans">
      ...
      <div className="w-full max-w-md bg-elevated-card border border-border-color p-8 rounded-3xl shadow-2xl z-10 animate-slide-in">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-secondary-text text-xs mt-1.5 font-outfit">Sign up to get started with Kaizen AI</p>
        </div>

        {/* Error Display Alert */}
        {localError && (
          <div className="mb-5 p-3.5 bg-red-950/20 border border-red-500/35 rounded-2xl text-xs font-semibold text-red-300 animate-fade-in font-outfit">
            {localError}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-bold tracking-widest font-mono uppercase text-muted-text">Your Name</label>
            <div className="relative flex items-center">
              <User className="absolute left-4 h-4 w-4 text-muted-text" />
              <input
                type="text"
                value={name}
        ...
        <div className="mt-6 text-center text-xs text-secondary-text">
          ...
          <Link to="/login" className="text-white hover:underline font-bold transition-colors">
            Log in
          </Link>
        </div>
      </div>
